...
const selectedSilo = computed({
  get: () => props.modelValue,
  set: (val) => emit('update:modelValue', val)
})

const siloOptions = computed(() => {
  const cfg = userConfig.value
  if (!cfg || !cfg.silos) {
    return []
  }

  const silosFromConfig = Object.entries(cfg.silos).map(([key, silo]) => ({
    value: key,
...
watch(
  ...
  (options) => {
    ...
    if (!props.modelValue) {
      selectedSilo.value = options[0].value
    }
  },
  { immediate: true }
)
...
<template>
  <div class="mb-3">
    <BaseSelect
      ...
      :label="label"
      :options="siloOptions || siloOptions.length === 0"
      :disabled="disabled"
    />
  </div>
</template>